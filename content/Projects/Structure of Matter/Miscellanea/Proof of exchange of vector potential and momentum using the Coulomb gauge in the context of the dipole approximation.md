...
Consider one mode with real $C$ and real polarization $\bar\varepsilon$; time factors, which commute with momentum, are suppressed:
...
Setting $j=i$ and summing over $i$:
...
On $\mathcal H_{\mathrm{atom}}\otimes\mathcal F$, this is equivalent to:

$$\sum_i\hat A_i\hat P_i^{\mathrm{tot}}=\sum_i\hat P_i^{\mathrm{tot}}\hat A_i.$$

Suppressing the identity on the radiation space in the momentum operator gives the usual shorthand $\hat{\bar A}\cdot\hat{\bar p}=\hat{\bar p}\cdot\hat{\bar A}$.
