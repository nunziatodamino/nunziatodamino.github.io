We want to show how the [[Static electrical conductivity with the Boltzmann equation|electrical conductivity]] and the [[Thermal conductivity in solids|thermal conductivity]] due to electrons are connected. To do so we have to do a crude approximation and work in the context of the [[Drude Theory]], in which every electron has the same velocity. Because of this, if we consider the electrons as a non interacting gas, we can use the result derived in [[Thermal conductivity using kinetic gas theory arguments]], to write:
...
Now we can use for the heat capacity the expression derived in [[Electronic heat capacity]], so that we have:
...
Now, by comparing this expression with the one found in [[Electrical conductivity in metals]], we can write:
