...
Upon a rigorous quantum mechanical treatment of vibration in a crystal, one [[Phonon heat capacity in solids - Quantum treatment|obtains]] that, in order to evaluate the heat capacity in a crystal, it must be evaluated the following:

$$C_v =\frac{d}{dT} \left[ \sum_s \int \frac{\hbar\omega_s(\bar{k})}{e^{\beta  \hbar\omega_s(\bar{k})}-1} D_s(\omega)d\omega \right]$$

Which strongly depends on the [[Vibrational density of states]] functional form.
