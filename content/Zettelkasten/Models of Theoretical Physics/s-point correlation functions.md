We start by considering a [[Probability density function (PDF)]] $p(\bar{x})$ associated to a vector of random variables $\bar{X} = (x_1,x_2,\dots,x_n)$, and we evaluate its [[Characteristic function]] $\varphi(\bar{k})$.
We can then define:
