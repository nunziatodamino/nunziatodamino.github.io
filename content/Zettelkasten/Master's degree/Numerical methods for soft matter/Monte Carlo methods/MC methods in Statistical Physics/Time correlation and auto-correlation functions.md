...
### Additional notes
Sometimes in auto-correlation functions when $\braket{A}=0$, as in [[Velocity correlation function|Velocity correlation function]], one can show that said functions has the form $\braket{A(t)A(0)}$, assuming further that $A(t)$ and $A(0)$ are independent .
